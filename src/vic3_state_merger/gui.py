...
def _validate_inputs(merge_file: str, mod_dir: str, small_state_limit: str) -> tuple[bool, str]:
    if not merge_file:
...
def main() -> None:
    ...
    ttk.Label(main_tab, text="Mod output folder").grid(row=1, column=0, sticky="w", pady=(8, 0))
    mod_entry = ttk.Entry(main_tab, textvariable=mod_dir_var)
    mod_entry.grid(row=1, column=1, sticky="ew", padx=(8, 8), pady=(8, 0))
    ttk.Button(main_tab, text="Browse...", command=lambda: _browse_folder(mod_dir_var)).grid(
        row=1, column=2, sticky="e", pady=(8, 0)
    )

    ttk.Label(main_tab, text="Small state limit").grid(row=2, column=0, sticky="w", pady=(8, 0))
    small_limit_entry = ttk.Entry(main_tab, textvariable=small_state_limit_var, width=8)
    small_limit_entry.grid(row=2, column=1, sticky="w", padx=(8, 8), pady=(8, 0))
    ...
    ignore_checkbox.grid(row=3, column=0, columnspan=2, sticky="w", pady=(8, 0))

    status_label = ttk.Label(main_tab, textvariable=status_var)
    status_label.grid(row=4, column=0, columnspan=3, sticky="w", pady=(12, 0))

    run_button = ttk.Button(main_tab, text="Run")
    run_button.grid(row=5, column=0, pady=(16, 0), sticky="w")
    ...
    def _run_merge() -> None:
        ...
        valid, error_message = _validate_inputs(merge_file, mod_dir, small_state_limit)
        if not valid:
